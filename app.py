...
if st.session_state.first_load and not st.session_state.loading_complete:
    ...
    # Display logo first - centered with slight right shift
    try:
        col1, col2, col3 = st.columns([0.8,2.4,0.8])
        with col2:
            st.image("PLP.png", width=250, use_column_width=False)
    except Exception as e:
        st.error(f"Logo loading error: {e}")
        st.markdown("PLP Logo", unsafe_allow_html=True)
    
    # Loading text
    st.markdown(
        """
        <div class="loading-text">PLP Support</div>
        <div class="loading-text">Data Processing System</div>
        """,
        unsafe_allow_html=True
    )
    
    # Progress bar and percentage with better visibility
    progress_placeholder = st.empty()
    percentage_placeholder = st.empty()
    
    # Simulate loading progress with better visibility
    for i in range(101):
        progress = i / 100
    ...
    # Mark loading as complete and transition smoothly
    st.session_state.loading_complete = True
    st.rerun()

# Main app (only shows after loading is complete)
if st.session_state.loading_complete:
    ...
    # Add smooth transition CSS for main app
    st.markdown(
        """
        <style>
        .stApp {
            animation: mainAppFadeIn 1.2s ease-in-out;
        }
    ...
    st.markdown(
        ...
        /* Hide any yellow components in main app */
        .stApp *[style*="background-color: yellow"],
        .stApp *[style*="background-color: #ffff00"],
        .stApp *[style*="background-color: #ffeb3b"],
        .stApp *[style*="background-color: #ffc107"],
        .stApp *[style*="background-color: #ff9800"] {
            display: none !important;
        }
        /* Hide any white text on colored backgrounds */
        .stApp *[style*="color: white"],
        .stApp *[style*="color: #ffffff"] {
            color: #222 !important;
        }
        /* Hide Streamlit's default toolbar/status elements */
